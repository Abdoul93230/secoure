...
const forgot_password_seller = async (req, res) => {
  ...
  if (email) {
    try {
      const existingUserByEmail = await SellerRequest.findOne({ email: email });
      if (existingUserByEmail) {
        // Générer un code OTP aléatoire
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
  ...
  <main>
    <div class="otp-container">
      ...
      <div class="contact-info">
        <div class="contact-item">
          <i class="fas fa-globe"></i>
          <a href="http://ihambaobab.onrender.com" target="_blank">ihambaobab.onrender.com</a>
        </div>
        <div class="contact-item">
          <i class="fas fa-phone"></i>
          <a href="tel:+22787727501">+227 87727501</a>
  ...
  if (otpCodes.get(email) === otp) {
    try {
      ...
      if (existingUserByEmail) {
        ...
        // Mettre à jour le mot de passe dans la base de données
        await SellerRequest.updateOne({ email: email }, { password: hash });
